...
exports.startQuiz = async (req, res) => {
  ...
  try {
    const user = await User.create({ name, email, category });
...
exports.submitQuiz = async (req, res) => {
  ...
  try {
    ...
    if (result === "fail") {
      ...
      // if (user.category === "pm")
        recommendation = "We are sad !! You didnt pass our initial test, Not to worry, our team will reach out on how you can join our Academy where you meet like minds like yourself to sharpen your skills.";
    } else {
      recommendation = "Congratulations, You pass our initial test. Please wait for the next steps as our team will reach out to you for a practical work to be done";
    }
    user.score = score;
    user.result = result;
    ...
    res.json({ score, total, result, recommendation });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
